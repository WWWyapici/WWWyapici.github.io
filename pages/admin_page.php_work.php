...
if (isset($_POST["new-entry"])) {
  ...
  $image_type = $upload_ext;
    ...
    if ($form_valid) {
      ...
      $db -> beginTransaction();
      echo 'plant_id_check'. $plant_id;

      $plant_id_check = exec_sql_query($db, "select * from plants where (plant_id = :plant_id);",
      array(
        ':plant_id' => $plant_id
      ));
      $plnts = $plant_id_check ->fetchAll();
